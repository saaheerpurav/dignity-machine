  ...
  useEffect(() => {
    if (running) {
      dotRefs.current.forEach(el => {
        if (el) gsap.set(el, { backgroundColor: '#e5dcc9', scale: 1 })
      })
      labelRefs.current.forEach(el => {
        if (el) gsap.set(el, { color: '#8b8276' })
      })
      ...
      STAGES.forEach((_, i) => {
        tl.to(dotRefs.current[i], {
          backgroundColor: '#3f5d4a', scale: 1.3, duration: 0.25, ease: 'back.out(2)',
        }, i * 4.5)
          .to(labelRefs.current[i], { color: '#3f5d4a', duration: 0.2 }, `<`)
  ...
  useEffect(() => {
    if (done) {
      ...
      dotRefs.current.forEach((el, i) => {
        if (el) gsap.to(el, { backgroundColor: '#a3bea1', scale: 1, duration: 0.25, delay: i * 0.06 })
      })
      labelRefs.current.forEach((el, i) => {
        if (el) gsap.to(el, { color: '#34503e', duration: 0.25, delay: i * 0.06 })
      })
              ...
              <Tooltip.Root>
                <Tooltip.Trigger asChild>
                  <div
                    ...
                    style={{ backgroundColor: !running && !done ? '#e5dcc9' : undefined }}
                  >
